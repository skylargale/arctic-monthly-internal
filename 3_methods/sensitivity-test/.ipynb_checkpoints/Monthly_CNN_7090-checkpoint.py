...
GEOMETRY_WEIGHTING = None if args.geometry == "none" else "cos"
...
# 1. Set `GEOMETRY_WEIGHTING = None` in the Configuration cell above, re-run the data-loading cells and the training loop cell. This is the baseline (matches the manuscript as submitted).
# 2. Set `GEOMETRY_WEIGHTING = 'cos'`, re-run the same cells. Results are saved with a `_cos` filename tag so they don't overwrite the baseline `.npy` files.
# 3. Run the cell below to compare the two.


def load_run(tag):
    obs = np.load(f'{output_dir}{region}_obs{tag}.npy')
    preds = [np.load(f'{output_dir}{region.lower()}_{name}{tag}.npy') for name in model_names_spliced]
    return obs, preds


baseline_obs, baseline_preds = load_run('')
try:
    weighted_obs, weighted_preds = load_run('_cos')
    ...
    print(f"Cos-latitude weighted     {months[MONTH_IDX]} mean predicted trend: {weighted_mean:.3f} K/dec")
    print(f'Difference:               {weighted_mean - baseline_mean:+.3f} K/dec')
except FileNotFoundError:
    print("Run the training loop once with GEOMETRY_WEIGHTING = 'cos' (step 2 above) to enable this comparison.")
